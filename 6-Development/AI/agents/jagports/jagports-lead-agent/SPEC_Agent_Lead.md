...
## Raspberry Pi reviewed-main synchronization (#934 / PR #935)

**Operational specification:** maintain the current `codex` user `jagports-lead-agent.timer` at 585 minutes, and use a **distinct daily user-systemd maintenance timer** to check whether the installed Lead Agent application subtree differs from reviewed repository `main`. The maintenance service must not constitute a second agent scheduler or run under `admin`; `codex` has no sudo and must never be granted it. Installation, first enrollment and the host-specific commands are owned by [Agent Update / OPERATIONS PR #935](https://github.com/jagports/jagports/pull/935) and the [MyNode deployment guide](../../../../../3-Deployment/hardware/RaspberryPI/MyNodeBTC/Jagports_Lead_Agent_Installation.md), not duplicated here.

**Safe scheduled behavior:** daily **check-only** by default, with explicit operator opt-in for auto-applying commits from `main` only after the repo's branch protection actually enforces the normal independent-review/required-CI gates and the Pi's first sync is enrolled. Capture exact SHA and managed code-tree identity; no open PR, local uncommitted change or unrelated GitHub branch may become an unattended update. Stage the new subtree separately, validate disabled-by-default paid execution and all offline tests without API tokens, record the deletion plan, and skip an update if the original agent service is active. Stop only the runtime timer while replacing code, back up the prior installation, preserve private `.env`, `venv`, `state` (including uncertain/cumulative model cost records and #904 cursors), `reports`, `notifications` and other approved local artifacts. Remove obsolete Vehicle/P7 code in the new reviewed release only after checking the prior managed-file manifest; refuse unknown/untracked deletions.

**Failure/acceptance:** verify one deterministic, paid-disabled post-update service run, report/state ownership and status before restoring the previously active 585-minute timer. If tests fail before a swap, leave the existing service intact; if post-swap validation fails, roll back code/config from the private backup, rerun offline tests and keep the runtime timer stopped if rollback is not verified. Changed updater executables or systemd units live outside the replaced application tree and require separate operator review. Logs expose commit, tested update outcome, rollback/blocked reason and journal evidence without tokens; periodic checks cannot silently introduce new paid API spending, independent Telegram polling or autonomous GitHub writes. CI and host acceptance must independently verify one no-change check, one available-update check, one controlled update and fail-closed rollback/permission behavior.
...
3. Correct `run-agent.sh` (currently invoking nonexistent `agent.py`) to invoke the same `main.py` as systemd; align the current README, operations and deployment instructions without overwriting the separate [Agent Update PR #935](https://github.com/jagports/jagports/pull/935).
