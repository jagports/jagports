...
2. Use the confirmed token name `jagports-lead-agent-ro` and expiry 366 days and **Resource owner: `jagports`**. Under Repository access select **Only select repositories** and choose **`jagports/jagports` only**. If organization approval is required, await approval before interpreting failed reads as permissions.
