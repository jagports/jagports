...
**Follow-up operator diagnostic:** A subsequent isolated authenticated GET of `/repos/jagports/jagports/issues/904` using `GITHUB_TOKEN_RO` returned **HTTP 200** on the Raspberry Pi. This establishes working Issue endpoint access at that time, but does not explain the earlier verifier's `approved_issue_read_failed` result; its JSON identity check and the write-denial test remain unverified. Before repeating the full verifier, validate the returned Issue number with one bounded redacted GET; do not mistake a transient or inconsistent API response for a verified write-denial result.
