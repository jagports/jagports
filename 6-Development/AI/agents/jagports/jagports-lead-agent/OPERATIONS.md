...
**Operator-confirmed live GitHub token settings:** The token details page reports `jagports-lead-agent-ro`, resource owner `jagports`, repository `jagports/jagports`, expiry **24 September 2027**, **Read access to code, issues and metadata**, and **no organization permissions**. These are the token's displayed settings, not evidence that its API access works. In the first one-shot Pi verification, `identity_authenticated=true`, `issue_read=false`, `issue_comment_write_denied=false`, `reason=approved_issue_read_failed`, `result=blocked`. The verifier stopped before its invalid-comment probe. **Do not enable the paid pilot or mark credential confirmation true.** Diagnose the failed Issue GET by collecting its HTTP status and approved diagnostic headers only (never the token, identity, full response body or Authorization header); do not repeat the entire verifier until the read failure is understood.

**Follow-up operator diagnostics (confirmed):** An isolated authenticated GET of `/repos/jagports/jagports/issues/904` using `GITHUB_TOKEN_RO` returned **HTTP 200** on the Raspberry Pi. The Product Owner then confirmed a separate bounded JSON check returned **HTTP 200** and `Correct Issue: True` for Issue #904. The earlier verifier's `approved_issue_read_failed` result is not explained by those successful checks, and the write-denial probe has **not** yet succeeded or been verified. A subsequent single full verifier run is the next acceptance step; do not activate paid model calls or claim the token's write prohibition yet.
...
**Verified read-only credential — Product Owner Pi evidence:** Using the corrected verifier containing the complete bounded response read, the operator reported `identity_authenticated=true`, `issue_read=true`, `issue_comment_write_denied=true`, `result=verified` for repository `jagports/jagports`. The write probe was a deliberately invalid empty Issue comment; GitHub rejected it with an explicit permission denial. No valid comment was submitted. This confirms the tested Issue-comment write boundary and complements the independently inspected fine-grained GitHub permissions (Metadata/Contents/Issues Read; no organization permissions). It does **not** prove unrelated GitHub endpoints or verify that `main.py` actually selects `GITHUB_TOKEN_RO`; that wiring and offline test remain acceptance gates before enabling the model-backed pilot. Keep the local paid switches disabled until then. The Pi evidence is redacted; token contents must never be copied to GitHub.
...
**Owner-confirmed compilation:** After fixing `scripts` ownership, the administrator ran `sudo -u codex` to execute `./venv/bin/python -m py_compile scripts/verify_readonly_github.py`; the operator confirmed `Compilation: PASS`. This validates cache creation and script compilation as the correct runtime user. The one-shot live GitHub permission test remains pending.
