...
async function handleApi(request, env) {
  ...
  if (path === "/api/stock" && request.method === "POST") {
    ...
    const result = await env.DB.prepare(
      ...
      partNumber,
      quantity,
      text(body.condition) || "unknown",
      text(body.status) || "available",
      text(body.location),
      text(body.donor_vehicle),
  ...
  if (stockMatch && request.method === "DELETE") {
    ...
    if (denied) return denied;
    const id = Number(stockMatch[1]);
    await env.DB.prepare("DELETE FROM stock_item WHERE id=?").bind(id).run();
    return json({ ok: true });
  }
  ...
  if (path === "/api/vehicles" && request.method === "POST") {
    ...
    const result = await env.DB.prepare(
      ...
      text(body.vin_raw),
      text(body.serial),
      text(body.model_range),
      text(body.market),
      text(body.identity_status) || "unresolved",
      text(body.notes)
    ).run();
    return json({ id: result.meta.last_row_id }, 201);
  }
