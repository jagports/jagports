...
import { estimateSource } from '../src/DataImporter.Estimate.mjs';
...
test('Range inventory is read-only, bounded to selected models and reproducibly sampled', async t => {
  ...
  const first = await estimateSource(options);
  ...
  const second = await estimateSource(options);
...
test('missing model directory and interrupted inventory never produce a complete projection', async t => {
  const options = await fixture(t);
  const missing = await estimateSource({ ...options, models: ['999'] });
  assert.equal(missing.result.state, 'INCOMPLETE');
  assert.equal(missing.result.inventory.errorCount, 1);
  let stop = false;
  const interrupted = await estimateSource(options, { onProgress: () => { stop = true; }, shouldStop: () => stop });
...
test('D1/time projection requires a measured calibration and keeps its basis explicit', async t => {
  ...
  const { result } = await estimateSource({ ...options, calibrationPath });
...
test('model-pattern estimate measures source scope without inventing a destination Range', async t => {
  const options = await fixture(t);
  const { result } = await estimateSource({ ...options, range: undefined, modelPattern: 'XK' });
  ...
  assert.match(result.projection.basis, /v0.1a/);
  await assert.rejects(estimateSource({ ...options, range: undefined, modelPattern: 'XK',
    calibrationPath: path.join(options.stateDir, 'calibration.json') }), /unavailable during source parsing/);
});
