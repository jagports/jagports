...
The operator's primary parsing input is a case-insensitive model-name pattern, for example `--parse XK`, `--parse XJ`, `--parse XJS`, `--parse X-Type`, `--parse X300` or `--parse X3`. Match it as a literal substring against the installed `models_l_id_0.xml` names and parent relationships, then process every complete category bundle in every matching leaf model. A parent name match includes its descendant leaves. Thus `X3` selects both X300 and X308, while the broader `XJ` also selects XJS; `XJS` selects only its own matching family. The resulting manifest records the exact query, Model_ID set, selected bundles and incomplete categories. There is no embedded Range-to-Model_ID mapping in this path. The existing forty-category XK selection remains an explicit bounded validation command; the primary `--parse` path is not limited to it. Both paths stage locally and do not publish to D1.
