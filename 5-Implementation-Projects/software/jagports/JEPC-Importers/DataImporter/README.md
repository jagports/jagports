...
`--parse PATTERN` reads the installed `menus/models_l_id_0.xml` on every run. It matches a case-insensitive literal substring of source model names, includes descendants when a parent row matches, and processes every complete category bundle under each matched leaf model. For example, `X3` matches X300 and X308; `XJ` also matches XJS, while `XJS` narrows to that family. There is no fixed model-ID mapping or sample limit. Incomplete categories are reported. The default source is `C:\Program Files\JEPC\applications\JEPC`; the default state directory is `%LOCALAPPDATA%\Jagports\JEPC-Importer`. Override either with `--source` or `--state-dir`.
