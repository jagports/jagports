...
The operator selects a known Range through the import scope (`select --range <slug>` in the CLI; the same choice belongs in a later UI). The selection manifest carries Range and model scope into `parse --manifest`; parsing must not ask for a second independent Range value. XK is the currently implemented bounded selection, and unsupported Range mappings must fail explicitly. Preserve compatibility with existing XK manifests that predate the explicit Range field.
