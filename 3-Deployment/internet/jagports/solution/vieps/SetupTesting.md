...
If the execution environment lacks Cloudflare access or the live hostname is unreachable, label only the affected **remote verification** BLOCKED/NOT RUN and retain the independently passed local/CI evidence. The [post-main runtime CI work](https://github.com/jagports/jagports/issues/633) remains separate until an actual post-deployment check has run with verified Worker revision and D1 state.
