...
Cloudflare Access is required before real inventory is exposed. `ADMIN_TOKEN` is only an MVP application-level authorization mechanism and does not replace Access. Production inventory and credentials must never be committed to GitHub.
