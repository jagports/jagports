...
## Scheduled reviewed-main updates (separate codex user timer)

The existing Lead Agent service/timer above remains the **only scheduler for executing the agent**. The optional `jagports-sync-main.timer` is a **separate maintenance timer**, introduced through [Agent Update PR #935](https://github.com/jagports/jagports/pull/935) and governed by [Issue #934](https://github.com/jagports/jagports/issues/934). It checks reviewed repository `main` daily at **04:15 Pi-local time**, plus up to ten minutes of randomized delay, with missed-check catch-up. This does not create another agent execution loop or change the existing 585-minute runtime timer.

**Default is check-only**: it logs main commit, installed commit and any application-code difference/deletion. Unattended installation from `main` requires separate explicit owner opt-in **after** verifying GitHub main branch protections/independent review and CI. An unmerged PR is never an update source. Only the approved application subtree is synchronized; old managed modules can be deleted after the initial manifest-based enrollment. No code auto-sync can guarantee that a public branch was independently reviewed unless repository protection actually enforces it.

The scheduled service is installed **as the existing `codex` account**, without sudo privileges, from the reviewed `scripts/sync_main.sh` and `systemd/jagports-sync-main.service`/`.timer` in the deployed application. The stable updater executable is copied to `/home/codex/.local/libexec/` **outside** the source tree it updates; the units live in `/home/codex/.config/systemd/user/`. The existing `codex` user manager and enabled linger must remain active. All host-admin operations continue to belong to `admin`; the automatic maintenance unit must never use sudo or run Python as admin.

For the **one-time installation, review, enrollment, daily-check verification, optional auto-deploy opt-in, disable procedure and recovery commands**, use the single canonical [OPERATIONS.md — Scheduled main-sync](../../../../6-Development/AI/agents/jagports/jagports-lead-agent/OPERATIONS.md#scheduled-main-sync-check-and-optional-unattended-application-update). Do not duplicate its deployment script here.

An automatic apply runs offline tests before swapping source, backs up the old code/config privately, retains `.env`, `venv/`, `state/`, `reports/` and `notifications/`, and verifies a model-disabled service invocation. If the runtime service is busy, the updater defers rather than interrupting it. A failed installed update attempts rollback; if rollback cannot be verified, the **agent timer remains stopped** pending operator repair. A successful update is not evidence that a later unattended 585-minute execution, live model-backed reasoning or Telegram delivery passed acceptance.
